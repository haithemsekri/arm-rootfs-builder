...
$WORKSPACE/arm-rootfs-builder/scripts/01-centos-7-arm32-base-rootfs.sh
elif [[ "$DISTRO-$TARGET_ARCH" == "centos-7-arm64" ]]; then
$WORKSPACE/arm-rootfs-builder/scripts/01-centos-7-arm64-base-rootfs.sh
elif [[ "$DISTRO-$TARGET_ARCH" == "ubuntu-18.04-arm64" ]]; then
$WORKSPACE/arm-rootfs-builder/scripts/02-ubuntu-18.04-arm64-base-rootfs.sh
else
echo "Invalid DISTRO: $DISTRO-$TARGET_ARCH"
exit 0
fi

$WORKSPACE/arm-rootfs-builder/scripts/03-sysroot-target-build.sh
