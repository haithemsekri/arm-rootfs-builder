...
[ -z $ROOTFS_DL_URL ]  && ROOTFS_DL_URL="http://ftp.rz.uni-frankfurt.de/pub/mirrors/centos-altarch/7.8.2003/isos/armhfp/CentOS-Userland-7-armv7hl-RootFS-Minimal-2003-sda.raw.xz"
...
if [ -f $ROOTFS_BASE_TAR ]; then
   ...
   LOOP_DEV="$(echo $PART | awk '{print $1}')"
...
EOF
   ...
   $SCRIPTS_DIR/15-extend-disk-image.sh $ROOTFS_BASE_DISK 256

   export ROOTFS_DISK_PATH=$ROOTFS_BASE_DISK
   source $SCRIPTS_DIR/12-chroot-run.sh
   chroot_umount_pseudo_fs
   sudo cp $SCRIPTS_DIR/01-centos-7-arm32-cpuinfo $RTFS_MNT_DIR/proc/cpuinfo
