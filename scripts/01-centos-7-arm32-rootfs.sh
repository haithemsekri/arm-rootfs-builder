...
[ -z $ROOTFS_DL_URL ]  && ROOTFS_DL_URL="http://ftp.rz.uni-frankfurt.de/pub/mirrors/centos-altarch/7.8.2003/isos/armhfp/CentOS-Userland-7-armv7hl-RaspberryPI-Minimal-2003-sda.raw.xz"

[ ! -f $ROOTFS_DL_FILE ] && wget $ROOTFS_DL_URL -O $ROOTFS_DL_FILE
[ ! -f $ROOTFS_DL_FILE ] && echo "$ROOTFS_DL_FILE : file not found" && exit 1
...
yum -y remove openssh-server grub2-common NetworkManager-wifi uboot-images-armv8 postfix chrony basesystem parted dracut-config-extradrivers sg3_utils man-db \
   ...
   kernel-tools NetworkManager-team grub2 selinux-policy-targeted passwd acl bind-export-libs libmnl libnetfilter_conntrack iproute dhcp-common net-tools extlinux-bootloader hostname \
   libnfnetlink sysvinit-tools iptables dhcp-libs
...
symlinks -c /usr/lib/gcc/armv7hl-redhat-linux-gnueabi/
echo "" > /etc/fstab
yum clean all

EOF

   $SCRIPTS_DIR/15-extend-disk-image.sh $ROOTFS_BASE_DISK 512
   export ROOTFS_DISK_PATH=$ROOTFS_BASE_DISK
   source $SCRIPTS_DIR/12-chroot-run.sh
   chroot_umount_pseudo_fs
   cp $SCRIPTS_DIR/01-centos-7-arm32-cpuinfo $RTFS_MNT_DIR/proc/cpuinfo
   chroot_run_script $CHROOT_SCRIPT
   sync
   rm -rf $CHROOT_SCRIPT
   rm -rf $RTFS_MNT_DIR/proc/cpuinfo
   cd $RTFS_MNT_DIR
